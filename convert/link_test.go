...
func (db *fakePathDbImpl) Get(fileId string) (path string, err error) {
	if fileId == "unexpected" {
		return "", newErrTransform(ERR_KIND_UNEXPECTED, "unexpected error generated by fakePathDbImpl")
	}
...
func TestWrapForReturningNotFoundPathError(t *testing.T) {
	cases := []struct {
		...
		},
		{
			fileId:   "unexpected",
			wantPath: "",
			wantErr:  newErrTransform(ERR_KIND_UNEXPECTED, ""),
		},
		{
			fileId:   "not_found",
			wantPath: "",
			wantErr:  newErrTransform(ERR_KIND_PATH_NOT_FOUND, ""),
		},
	}
