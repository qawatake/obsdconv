...
func TestSplitFragments(t *testing.T) {
	cases := []struct {
		...
		{identifier: "211022# section", wantFileId: "211022", wantFragments: []string{"section"}, wantErr: nil},
		{identifier: "211022# section # subsection", wantFileId: "211022", wantFragments: []string{"section", "subsection"}, wantErr: nil},
		{identifier: "211022 #section", wantFileId: "", wantFragments: nil, wantErr: newErrTransformf(ERR_KIND_INVALID_INTERNAL_LINK_CONTENT, "")},
		{identifier: "211022\t#section #subsection", wantFileId: "", wantFragments: nil, wantErr: newErrTransformf(ERR_KIND_INVALID_INTERNAL_LINK_CONTENT, "")},
	}
...
func TestPathDBGet(t *testing.T) {
	...
	for _, tt := range cases {
		db := NewPathDB(filepath.Join(testFindPathRootDir, tt.root))
		got, err := db.Get(tt.fileId)
		if err != nil {
...
type fakePathDbImpl struct{}

func (db *fakePathDbImpl) Get(fileId string) (path string, err error) {
	if fileId == "unexpected" {
		return "", newErrTransformf(ERR_KIND_UNEXPECTED, "unexpected error generated by fakePathDbImpl")
	}
	if fileId == "test.md" || fileId == "test" {
		return "test.md", nil
	}
	return "", nil
}

func TestWrapForReturningNotFoundPathError(t *testing.T) {
	cases := []struct {
		fileId   string
		wantPath string
		wantErr  ErrTransform
	}{
		{
			fileId:   "test",
			wantPath: "test.md",
		},
		{
			fileId:   "unexpected",
			wantPath: "",
			wantErr:  newErrTransformf(ERR_KIND_UNEXPECTED, ""),
		},
		{
			fileId:   "not_found",
			wantPath: "",
	...
	for _, tt := range cases {
		db := WrapForReturningNotFoundPathError(new(fakePathDbImpl))
		gotPath, err := db.Get(tt.fileId)
		...
		if gotPath != tt.wantPath {
			t.Errorf("[ERROR] got: %q, want: %q", gotPath, tt.wantPath)
		}
	}
}
